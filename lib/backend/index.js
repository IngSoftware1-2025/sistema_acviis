...
const vehiculosPermisosRoutes = require('./routes/vehiculos/vehiculos_permisos_mongoDB');
...
const obraRecursosRoutes = require('./routes/obra_recursos.js');
const obraFinanzasRoutes = require('./routes/obra_finanzas.js')

const { enviarRecordatoriosCharlas } = require('./routes/charlas/cron_charlas.js');
const { EnviarNotificacionesItemizados } = require('./routes/cron_itemizados.js');
const historialAsistenciaRoutes = require('./routes/historial_asistencia.js');
const { verificarCajasChicas } = require('./routes/finanzas/cron_cajas_chicas.js');

// Ruta para historial de asistencia
app.use('/historial-asistencia', historialAsistenciaRoutes); // /historial-asistencia/upload
...
app.use('/vehiculos-permisos', vehiculosPermisosRoutes);
...
app.use('/obra-recursos', obraRecursosRoutes);
app.use('/obra-finanzas', obraFinanzasRoutes)
...
app.listen(port, async () => {
  ...
  try {
    await enviarRecordatoriosCharlas();
  } catch (error) {
    console.error('[ERROR] Enviando recordatorios de charlas al iniciar:', error);
  }

  // Notificaciones de gastos itemizados
  try {
    await EnviarNotificacionesItemizados();
  } catch (error) {
    console.error('[ERROR] Enviando notificaciones de gastos itemizados:', error);
  }
  // Verificación de Cajas Chicas
  try {
    await verificarCajasChicas();
  } catch (error) {
    console.error('[ERROR] Verificando cajas chicas al iniciar:', error);
  }   
});
