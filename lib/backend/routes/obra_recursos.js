...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    console.log(`[DEBUG] Tipo de recurso: ${recursoTipo}`);
    console.log(`[DEBUG] VehiculoId recibido: ${vehiculoId}`);
    ...
    let idValido = false;
    let recursoData = null; // Para almacenar datos del recurso
    ...
    switch(recursoTipo) {
      ...
      case 'herramienta':
        if (herramientaId) {
          ...
          recursoData = herramienta;
          
          // VALIDACIÓN ESPECÍFICA PARA HERRAMIENTAS: Verificar cantidad disponible
          if (herramienta) {
            const cantidadSolicitada = cantidad || 1;
            if (cantidadSolicitada > herramienta.cantidad_disponible) {
              return res.status(400).json({ 
                error: `No hay suficiente cantidad disponible. Disponible: ${herramienta.cantidad_disponible}, Solicitado: ${cantidadSolicitada}` 
              });
            }
          }
        }
      ...
      case 'epp':
        if (eppId) {
          ...
          // VALIDACIÓN ESPECÍFICA PARA EPP: Verificar cantidad disponible
          if (epp) {
            const cantidadSolicitada = cantidad || 1;
            if (cantidadSolicitada > epp.cantidad_disponible) {
              return res.status(400).json({ 
                error: `No hay suficiente cantidad disponible. Disponible: ${epp.cantidad_disponible}, Solicitado: ${cantidadSolicitada}` 
              });
            }
          }
        }
    ...
    if (asignacionExistente) {
      ...
      try {
        ...
        const cantidadAsignar = cantidad || asignacionExistente.cantidad;
        ...
        // Si es herramienta, actualizar cantidad_disponible
        if (recursoTipo === 'herramienta' && herramientaId) {
          await prisma.herramientas.update({
            where: { id: herramientaId },
            data: {
              cantidad_disponible: {
                decrement: cantidadAsignar
              }
            }
          });
        }
        
        // Si es EPP, actualizar cantidad_disponible
        if (recursoTipo === 'epp' && eppId) {
          await prisma.epp.update({
            where: { id: parseInt(eppId) },
            data: {
              cantidad_disponible: {
                decrement: cantidadAsignar
              }
        ...
        // Si es un vehículo, actualizar el campo obra_asig
        if (recursoTipo === 'vehiculo' && vehiculoId) {
          console.log(`[DEBUG REASIGNACION] Actualizando vehículo ${vehiculoId} con obra: ${obraExiste.nombre}`);
          const vehiculoActualizado = await prisma.vehiculos.update({
            where: { id: vehiculoId },
            data: { obra_asig: obraExiste.nombre }
          });
          console.log(`[DEBUG REASIGNACION] Vehículo actualizado exitosamente:`, vehiculoActualizado);
        }
    ...
    try {
      const cantidadAsignar = cantidad || 1;
      
      // Si es herramienta, actualizar cantidad_disponible ANTES de crear la asignación
      if (recursoTipo === 'herramienta' && herramientaId) {
        await prisma.herramientas.update({
          where: { id: herramientaId },
          data: {
      ...
      // Si es EPP, actualizar cantidad_disponible ANTES de crear la asignación
      if (recursoTipo === 'epp' && eppId) {
        await prisma.epp.update({
          where: { id: parseInt(eppId) },
          data: {
      ...
      // Si es un vehículo, actualizar el campo obra_asig
      if (recursoTipo === 'vehiculo' && vehiculoId) {
        console.log(`[DEBUG NUEVA ASIGNACION] Actualizando vehículo ${vehiculoId} con obra: ${obraExiste.nombre}`);
        const vehiculoActualizado = await prisma.vehiculos.update({
          where: { id: vehiculoId },
          data: { obra_asig: obraExiste.nombre }
        });
        console.log(`[DEBUG NUEVA ASIGNACION] Vehículo actualizado exitosamente:`, vehiculoActualizado);
      }
...
router.put('/:id/retirar', async (req, res) => {
  ...
  try {
    ...
    // Si es herramienta, devolver la cantidad a cantidad_disponible
    if (asignacion.recurso_tipo === 'herramienta' && asignacion.herramienta_id) {
      await prisma.herramientas.update({
        where: { id: asignacion.herramienta_id },
        data: {
          cantidad_disponible: {
            increment: asignacion.cantidad
          }
        }
      });
    }
    
    // Si es EPP, devolver la cantidad a cantidad_disponible
    if (asignacion.recurso_tipo === 'epp' && asignacion.epp_id) {
      await prisma.epp.update({
        where: { id: asignacion.epp_id },
        data: {
          cantidad_disponible: {
            increment: asignacion.cantidad
          }
    ...
    // Si es un vehículo, actualizar obra_asig a "Sin obra asignada"
    if (asignacion.recurso_tipo === 'vehiculo' && asignacion.vehiculo_id) {
      console.log(`[DEBUG RETIRO] Actualizando vehículo ${asignacion.vehiculo_id} a "Sin obra asignada"`);
      const vehiculoActualizado = await prisma.vehiculos.update({
        where: { id: asignacion.vehiculo_id },
        data: { obra_asig: "Sin obra asignada" }
      });
      console.log(`[DEBUG RETIRO] Vehículo actualizado exitosamente:`, vehiculoActualizado);
    }
...
router.get('/disponibles/:tipo', async (req, res) => {
  ...
  try {
    ...
    switch(tipo) {
      ...
      case 'herramienta':
        recursosDisponibles = await prisma.herramientas.findMany({
          where: {
            ...
            cantidad_disponible: {
              gt: 0 // Solo herramientas con cantidad disponible mayor a 0
            }
          },
          select: {
            id: true,
      ...
      case 'epp':
        // Para EPP, filtrar los que tienen cantidad disponible
        recursosDisponibles = await prisma.epp.findMany({
          where: {
            cantidad_disponible: {
              gt: 0 // Solo EPP con cantidad disponible mayor a 0
            }
          },
          select: {
            id: true,
