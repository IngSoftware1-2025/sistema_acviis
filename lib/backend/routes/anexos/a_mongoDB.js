...
router.get('/descargar-pdf/:id', async (req, res) => {
    try {
        ...
        console.log('[DEBUG] Buscando PDF con id_anexo:', id, 'Archivos encontrados:', files.length);
        ...
        if (!files || files.length === 0) {
            ...
            console.log('[DEBUG] Buscando PDF por filename:', id, 'Archivos encontrados:', files.length);
        }
        if (!files || files.length === 0) {
            filesCursor = bucket.find({ filename: id + '.pdf' });
            files = await filesCursor.toArray();
            console.log('[DEBUG] Buscando PDF por filename:', id + '.pdf', 'Archivos encontrados:', files.length);
        }
