...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!numero_orden || !fecha_emision || !proveedor_id || !centro_costo || !nombre_servicio || valor === undefined) {
    ...
    const ordenActualizada = await prisma.ordenes_de_compra.update({
      ...
      data: {
        numero_orden,
        fecha_emision: new Date(fecha_emision), // Asegurarse de que la fecha sea un objeto Date
        proveedorId: proveedor_id, // CORRECCIÓN: El campo en el modelo es 'proveedorId'
        centro_costo,
        seccion_itemizado,
        numero_cotizacion,
        numero_contacto,
    ...
    // Manejo de errores de Prisma (ej: si no encuentra el registro)
    if (error.code === 'P2025') {
