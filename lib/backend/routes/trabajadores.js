...
router.get('/', async (req, res) => {
  try {
    const trabajadores = await prisma.trabajadores.findMany({
      orderBy: { id: 'desc' },
      include: { contratos: true }, // Asegúrate de que la relación 'contratos' existe en tu modelo Prisma
    });
...
router.put('/:id/datos', async (req, res) => {
  ...
  if (
    ...
    !prevision_afp || !obra_en_la_que_trabaja || !rol_que_asume_en_la_obra || !estado_trabajador
  ) {
