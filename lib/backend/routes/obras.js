...
router.put('/:id', async (req, res) => {
  ...
  for (const fields of requiredFields) {
    if (req.body[fields] === undefined || req.body[fields] === null) {
        return res.status(400).json({ error: `El campo ${fields} es obligatorio.` });
    }
  }
  try {
    console.log('Actualizando obra con datos:', req.body);
    
    const obraActualizada = await prisma.obras.update({
      where: { id },
      data: {
        nombre,
        descripcion,
        responsable_email,
        direccion,
        obraInicio: obraInicio ? new Date(obraInicio) : null,
        obraFin: obraFin ? new Date(obraFin) : null,
        jornada
      }
    });
    
    console.log('Obra actualizada exitosamente:', obraActualizada.id);
    res.json(obraActualizada);
  } catch (error) {
    console.error(`Error al actualizar la obra ${id}:`, error);
